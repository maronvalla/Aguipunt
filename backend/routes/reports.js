...
const reportFiltersDefault = require("../services/reportFilters");
const dailyTotalsDefault = require("../services/dailyTotals");

const resolveReportFilters = (req, buildDailyRange) => {
...
const createReportsRouter = (deps = {}) => {
  const db = deps.db || dbDefault;
  const requireRole = deps.requireRole || requireRoleDefault;
  const buildTransactionsFilters =
    deps.buildTransactionsFilters || reportFiltersDefault.buildTransactionsFilters;
  const buildDailyRange =
    deps.buildDailyRange || reportFiltersDefault.buildDailyRange;
  const getDailyTotals = deps.getDailyTotals || dailyTotalsDefault.getDailyTotals;
  ...
  router.get(
    ...
    requireRole("admin"),
    async (req, res) => {
      const { from, to, userIdRaw, userId, userName } = resolveReportFilters(
        req,
        buildDailyRange
      );
    ...
    async (req, res) => {
      ...
      try {
        ...
        try {
          ...
          console.warn(
            "[reports] redeem report fallback mode enabled",
            queryErr?.message
          );
        }
        ...
        const totals = items.reduce(
          (acc, item) => {
            ...
            acc.usersWithRedemptions +=
              item.customRedeemedPoints > 0 || item.prizeRedemptionsCount > 0
                ? 1
                : 0;
            return acc;
          },
